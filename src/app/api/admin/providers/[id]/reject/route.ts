...
export async function PUT(
  ...
  try {
    gatewayResponse = await fetch(`${GATEWAY_URL}/api/providers/admin/${id}/reject`, {
      method: "PUT",
      headers: { Authorization: `Bearer ${token}` },
    });
  } catch (err) {
    console.error("rejectProvider: failed to reach gateway", err);
    return NextResponse.json(
  ...
  if (!gatewayResponse.ok || !payload.success) {
    return NextResponse.json(
      { success: false, message: payload.message || "Failed to reject provider" },
