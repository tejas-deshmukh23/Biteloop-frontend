...
export async function GET(request: NextRequest) {
  ...
  try {
    gatewayResponse = await fetch(`${GATEWAY_URL}/api/providers/admin/pending`, {
      method: "GET",
      headers: { Authorization: `Bearer ${token}` },
    });
  } catch (err) {
    console.error("getPendingProviders: failed to reach gateway", err);
    return NextResponse.json(
  ...
  if (!gatewayResponse.ok || !payload.success) {
    return NextResponse.json(
      { success: false, message: payload.message || "Failed to fetch pending providers" },
