...
export async function DELETE(
  ...
  try {
    gatewayResponse = await fetch(`${GATEWAY_URL}/api/orders/${id}/cancel`, {
      method: "DELETE",
      headers: { Authorization: `Bearer ${token}` },
    });
  } catch (err) {
    console.error("cancelOrder: failed to reach gateway", err);
    return NextResponse.json(
  ...
  if (!gatewayResponse.ok || !payload.success) {
    return NextResponse.json(
      { success: false, message: payload.message || "Failed to cancel order" },
