...
export async function GET(
  ...
  try {
    gatewayResponse = await fetch(`${GATEWAY_URL}/api/orders/${id}`, {
      method: "GET",
      headers: { Authorization: `Bearer ${token}` },
    });
  } catch (err) {
    console.error("getOrderById: failed to reach gateway", err);
    return NextResponse.json(
  ...
  if (!gatewayResponse.ok || !payload.success) {
    return NextResponse.json(
      { success: false, message: payload.message || "Failed to fetch order" },
